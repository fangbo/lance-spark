...
- When `spark.sql.lance.rewrite_columns` is set to `true` (the default behavior), `MERGE INTO ... WHEN MATCHED UPDATE` operation will attempt to perform column-level updates. Instead of deleting the matched rows and inserting new versions, the engine will only write new versions of the changed columns.
- When the configuration is set to `false`, the operation fall back to rewriting the affected rows (a "delete and insert" operation).
