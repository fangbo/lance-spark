...
- When `spark.sql.lance.rewrite_columns` is set to `true` (the default behavior), `UPDATE` and `MERGE INTO ... WHEN MATCHED UPDATE` operations will attempt to perform column-level updates. Instead of deleting the matched rows and inserting new versions, the engine will only write new versions of the changed columns.
- When the configuration is set to `false`, the operations fall back to rewriting the affected rows (a "delete and insert" operation).
- This feature applies to both `UPDATE` statements and the `UPDATE` clause of `MERGE INTO` statements.
...
When enabled, the `UPDATE` clause in a `MERGE INTO` statement will also benefit from this optimization.
...
- **Spark Version**: `UPDATE` and `MERGE INTO` operations are supported on Spark 3.5 and newer.
